 ...
 * stepped on.
 * <p>
 * Gives the Main Character a trap powerup when stepped on
 *
 * @author Bryan
 * @version 1.0
 * @see Pickup_Trap_Inactive
 */
public class Pickup_Trap extends ScoreCell{

    /**
     * Initializes the CellType and Score
     */
    private void paramList() {
        cellEnum = CellType.Pickup_Trap;
        score = 50;
    }
     ...
     */
    public Pickup_Trap(GroundType setGround, Pair<Integer, Integer> setPosition, MapManager setManager) {
        this.paramList();
    ...
    /**
     * Creates a new Pickup_Trap_Inactive object and inserts it into the calling
     * Pickup_Trap's position in the cell array. Adds the Inactive cell to the
    ...
    public void updateCellState() {
        Pickup_Trap_Inactive toAdd = new Pickup_Trap_Inactive(groundEnum, cellPosition, mapManager);
        MapManager.cellArray[this.cellPosition.getKey()][this.cellPosition.getValue()] = toAdd;
        mapManager.addToTimer(toAdd);

        // Adds Pickup_Trap to the resetHandler
